...
var require_meta = __commonJSMin((exports, module) => {
	(function(mod) {
		...
		CodeMirror$1.findModeByMIME = function(mime) {
			...
			if (mime.endsWith('+xml')) return CodeMirror$1.findModeByMIME("application/xml");
			if (mime.endsWith('+json')) return CodeMirror$1.findModeByMIME("application/json");
		};
			...
			return {
				...
				indent: function(state, textAfter, line) {
					...
					if (cx.prev && (ch$1 == "}" && (cx.type == "block" || cx.type == "atBlock" || cx.type == "keyframes") || ch$1 == ")" && (cx.type == "parens" || cx.type == "atBlock_parens") || ch$1 == "{" && cx.type == "at")) indent = cx.indent - indentUnit;
						...
						if (/@|\$|\d/.test(ch$1) || textAfter.startsWith('{') || /^\s*\/(\/|\*)/.test(textAfter) || /^\s*\/\*/.test(prevLineFirstWord) || /^\s*[\w-.[\]'"]+\s*(\?|:|\+)?=/i.test(textAfter) || /^(\+|-)?[a-z][\w-]*\(/i.test(textAfter) || textAfter.startsWith('return') || wordIsBlock(lineFirstWord)) indent = lineIndent;
